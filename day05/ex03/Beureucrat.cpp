...
int	Beureucrat::get_grade(void) const
{
	return (this->grade);
}

const std::string	Beureucrat::get_name(void) const
{
	return (this->name);
}
...
void	Beureucrat::sign_form(Form &form)
{
	if (!form.get_sign())
	{
		...
		{
			std::cout << this->name << " cannot sign form '" << form.get_name() << "' because he's not that important" << std::endl;
		}
	}
	else
	{
